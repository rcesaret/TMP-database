...
complexData <- data %>% 
  select(names(data_list[["Data_complexData"]] %>% select(-SSN)))

complexMacroData <- data %>% 
  select(names(data_list[["Data_complexMacroData"]] %>% select(-SSN)))

archInterp <- data %>% 
  select(names(data_list[["Data_archInterp"]] %>% select(-SSN)))

archMaterial <- data %>% 
  select(DF9_metadata %>% filter(dbTable_DF9b == "Data_archMaterial") %>% select(varName_DF9b) %>% pull())

description <- data %>% 
  select(names(data_list[["Data_description"]] %>% select(-SSN)))

workshop <- data %>% 
  select(names(data_list[["Data_workshop"]] %>% select(-SSN)))

admin <- data %>% 
  select(names(data_list[["Data_admin"]] %>% select(-SSN)))

dataReportData_list <- list(complexData = complexData, complexMacroData = complexMacroData, archInterp = archInterp, archMaterial = archMaterial, description = description, workshop = workshop, admin = admin)

dataReportData_list_f <- lapply(dataReportData_list, function(df) {
