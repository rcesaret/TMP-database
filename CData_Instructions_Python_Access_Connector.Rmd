...
**Overview**
The CData Python Connector for Microsoft Access allows developers to write Python scripts with connectivity to Microsoft Access. The connector wraps the complexity of accessing Microsoft Access data in an interface commonly used by python connectors to common database systems.
...
-   Write and execute SQL queries to fetch and update data in Microsoft Access.
...
See Using from Tools to connect Microsoft Access data to tools such as Pandas or Petl.

**SQLAlchemy ORM**
SQLAlchemy can be leveraged to model the tables in Microsoft Access with mapped classes. See From SQLAlchemy for instructions for configuring the Python connector with SQLAlchemy.
...
**Advanced Features**
Advanced Features details additional features supported by the connector, such as , ssl configuration, remoting, firewall/proxy settings, and advanced logging.


**Data Model**
See Data Model for the available database objects. This section also provides more detailed information on querying specific Microsoft Access entities.
...
This property restricts the schemas reported to a subset of the available schemas. For example, BrowsableSchemas=SchemaA,SchemaB,SchemaC. Listing the schemas from databases can be expensive. Providing a list of schemas in the connection string improves the performance.
  * **Data Type:** string
  * **Default Value:** ""

#### 7.3.3. Tables

This property restricts the tables reported to a subset of the available tables. For example, Tables=TableA,TableB,TableC. Listing the tables from some databases can be expensive. Providing a list of tables in the connection string improves the performance of the connector. This property can also be used as an alternative to automatically listing views if you already know which ones you want to work with and there would otherwise be too many to work with. Specify the tables you want in a comma-separated list. Each table should be a valid SQL identifier with any special characters escaped using square brackets, double-quotes or backticks. For example, Tables=TableA,[TableB/WithSlash],WithCatalog.WithSchema.`TableC With Space`.
  * **Data Type:** string
  * **Default Value:** ""
...
Restricts the views reported to a subset of the available tables. For example, Views=ViewA,ViewB,ViewC. Listing the views from some databases can be expensive. Providing a list of views in the connection string improves the performance of the connector. This property can also be used as an alternative to automatically listing views if you already know which ones you want to work with and there would otherwise be too many to work with. Specify the views you want in a comma-separated list. Each view should be a valid SQL identifier with any special characters escaped using square brackets, double-quotes or backticks. For example, Views=ViewA,[ViewB/WithSlash],WithCatalog.WithSchema.`ViewC With Space`. Note that when connecting to a data source with multiple schemas or catalogs, you will need to provide the fully qualified name of the table in this property, as in the last example here, to avoid ambiguity between tables that exist in multiple catalogs or schemas.
  * **Data Type:** string
  * **Default Value:** ""
...
Automatically caches the results of SELECT queries into a cache database specified by either CacheLocation or both of CacheConnection and CacheProvider. When AutoCache = true, the connector automatically maintains a cache of your table's data in the database of your choice.
  * **Data Type:** bool
  * **Default Value:** false

##### Setting the Caching Database

When <u>AutoCache</u> = true, the connector caches to a simple, file-based cache. You can configure its location or cache to a different database with the following properties:

-   [CacheLocation](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheLocation.htm): Specifies the path to the file store.
-   [CacheProvider](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheProvider.htm) and [CacheConnection](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheConnection.htm): Specifies a driver to a database and the connection string.

##### See Also

-   [CacheMetadata](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheMetadata.htm): This property reduces the amount of metadata that crosses the network by persisting table schemas retrieved from the Microsoft Access metadata. Metadata then needs to be retrieved only once instead of every connection.
-   [Explicitly Caching Data](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_AutoCache.htm#pg_cacheExplicitly): This section provides more examples of using <u>AutoCache</u> in [Offline](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_Offline.htm) mode.
-   [CACHE Statements](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_AutoCache.htm#pg_cache): You can use the CACHE statement to persist any SELECT query, as well as manage the cache; for example, refreshing schemas.


#### 7.4.2. CacheProvider

The name of the provider to be used to cache data. You can cache to any database for which you have an ADO.NET provider. The caching database is determined based on the CacheProvider and [CacheConnection](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheConnection.htm) properties.
  * **Data Type:** string
  * **Default Value:** ""

The following sections show connection examples and address other requirements for several popular database drivers. Refer to [CacheConnection](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheConnection.htm) for more information on typical connection properties.
...
The database driver used to cache data. You can cache to any database for which you have a JDBC driver, including CData JDBC drivers. The cache database is determined by the <u>CacheDriver</u> and [CacheConnection](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheConnection.htm) properties. The <u>CacheDriver</u> is the name of the JDBC driver class that you want to use to cache data.
  * **Data Type:** string
  * **Default Value:** ""

**Note:** you must add the <u>CacheDriver</u> JAR file to the classpath. For Linux systems and macOS, you need to create a config.ini file on the installation path of the driver (site-packages/cdata). The config.ini file has the following format (the driver and the path of the JDBC driver):  
...
The following examples show how to cache to several major databases. Refer to [CacheConnection](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheConnection.htm) for more information on the JDBC URL syntax and typical connection properties.

##### PostgreSQL

The following JDBC URL uses the official PostgreSQL JDBC driver:  

`jdbc:access:CacheDriver=cdata.jdbc.postgresql.PostgreSQLDriver;CacheConnection='jdbc:postgresql:User=postgres;Password=admin;Database=postgres;Server=localhost;Port=5432;';DataSource=C:\\My.accdb`
...
The cache database is determined based on the CacheProvider and CacheConnection properties. Both properties are required to use the cache database. Examples of common cache database settings can be found below. For more information on setting the caching database's driver, refer to CacheProvider. The connection string specified in the CacheConnection property is passed directly to the underlying CacheProvider. Consult the documentation for the specific for more information on the available properties. 
  * **Data Type:** string
  * **Default Value:** ""
...
Specifies the path to the cache when caching to a file. The CacheLocation is a simple, file-based cache.
  * **Data Type:** string
  * **Default Value:** "%APPDATA%\\CData\\Access Data Provider"

If left unspecified, the default location is "%APPDATA%\\CData\\Access Data Provider" with %APPDATA% being set to the user's configuration directory:

<table><tbody><tr><td><b>Platform</b></td><td><b>%APPDATA%</b></td></tr><tr><td>Windows</td><td>The value of the APPDATA environment variable</td></tr><tr><td>Linux</td><td>~/.config</td></tr></tbody></table>

##### See Also

-   [AutoCache](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_AutoCache.htm): Set to implicitly create and maintain a cache for later offline use.
-   [CacheMetadata](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheMetadata.htm): Set to persist the Microsoft Access catalog in <u>CacheLocation</u>.

#### 7.4.6. CacheTolerance

The tolerance for stale data in the cache specified in seconds when using AutoCache. The tolerance for stale data in the cache specified in seconds. This only applies when AutoCache is used. The connector checks with the data source for newer records after the tolerance interval has expired. Otherwise, it returns the data directly from the cache.
  * **Data Type:** int
  * **Default Value:** 600

#### 7.4.7. Offline

Use offline mode to get the data from the cache instead of the live source. When Offline = true, all queries execute against the cache as opposed to the live data source. In this mode, certain queries like INSERT, UPDATE, DELETE, and CACHE are not allowed.
  * **Data Type:** bool
  * **Default Value:** false

#### 7.4.8. CacheMetadata

This property determines whether or not to cache the table metadata to a file store. As you execute queries with this property set, table metadata in the Microsoft Access catalog is cached to the file store specified by CacheLocation if set or the user's home directory otherwise. A table's metadata is retrieved only once, when the table is queried for the first time.
  * **Data Type:** bool
  * **Default Value:** false
...
-   **When you are caching to a database**: <u>CacheMetadata</u> can only be used with [CacheLocation](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheLocation.htm). If you are caching to another database with the [CacheProvider](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheProvider.htm) and [CacheConnection](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_CacheConnection.htm) properties, use [AutoCache](https://cdn.cdata.com/help/DCK/py/RSBAccess_p_AutoCache.htm) to cache implicitly.
...
Specifies what format shouLd be used by a new database file created when the file specified by connection property DataSource does not exist. By default this is set to No, and an exception will be thrown instead of creating new database file. When a new database file is created it's file name extension is forcibly specified to correspond to default file name extension used by respective database's format pointed by this property.
  * **Data Type:** string
  * **Default Value:** "No"
  * **Possible Values:** No, 2003, 2007, 2010, 2016, 2019

#### 7.5.2. IncludeMSysTables

Set this property to the true value to allow querying from the system tables. Tells provider to expose system tables in the read-only mode. Default value is False.
  * **Data Type:** bool
  * **Default Value:** false
...
These hidden properties are used only in specific use cases. The properties listed below are available for specific use cases. Normal driver use cases and functionality should not require these properties. Specify multiple properties in a semicolon-separated list.
  * **Data Type:** string
  * **Default Value:** ""

##### Integration and Formatting

<table><tbody><tr><td>DefaultColumnSize</td><td>Sets the default length of string fields when the data source does not provide column length in the metadata. The default value is 2000.</td></tr><tr><td>ConvertDateTimeToGMT</td><td>Determines whether to convert date-time values to GMT, instead of the local time of the machine.</td></tr><tr><td>RecordToFile=filename</td><td>Records the underlying socket data transfer to the specified file.</td></tr></tbody></table>

#### 7.5.5. Readonly

You can use this property to enforce read-only access to Microsoft Access from the provider. If this property is set to true, the connector will allow only SELECT queries. INSERT, UPDATE, DELETE, and stored procedure queries will cause an error to be thrown.
  * **Data Type:** bool
  * **Default Value:** false

#### 7.5.6. RTK

The runtime key used for licensing. The RTK property may be used to license a build.
  * **Data Type:** string
  * **Default Value:** ""
